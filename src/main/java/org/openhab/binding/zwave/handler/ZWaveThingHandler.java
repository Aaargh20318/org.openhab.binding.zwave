...
import org.openhab.binding.zwave.internal.ZWaveHandlerFactory;
...
public class ZWaveThingHandler extends ConfigStatusThingHandler implements ZWaveEventListener {
	...
	public void initialize() {
		...
		ZWaveHandlerFactory.getBridges().forEach(new Consumer<ZWaveSerialHandler>() {
			@Override
			public void accept(ZWaveSerialHandler f) {
				f.addEventListener(ZWaveThingHandler.this);
			}
		});
	}
			...
			for (ZWaveThingChannel channel : thingChannelsState) {
				...
				if (state != null) {
					...
					if(event instanceof ZWaveCommandClassValueEvent) {
						ZWaveCommandClassValueEvent ccve = (ZWaveCommandClassValueEvent) event;
						if(ccve.getCommandClass() == CommandClass.BASIC && ccve.getType() instanceof Integer && (Integer)ccve.getType() == ZWaveBasicCommandClass.BASIC_SET) {
							String st = state.toFullString();
							if(st.equals("0") ) {
								st = "DIMMEROFF";
							} else if(st.equals("100")) {
								st= "DIMMERON";
							}
							String eventMessage = st+"_"+ccve.getMessageSource();
							ChannelUID uid = new ChannelUID(getThing().getUID(),"event");
							triggerChannel(uid, eventMessage);
							logger.debug("NODE {}: triggered event for channel {} with event {} ", nodeId, uid, eventMessage);
						} else {
							logger.debug("NODE {}: ignoring command with class {}, command {}",nodeId,ccve.getCommandClass().name(),ccve.getType());
						}
					}
